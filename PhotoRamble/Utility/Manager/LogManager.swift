...
internal final class LogManager {
  ...
  internal enum LogCategory: String {
    ...
    internal static let bundleID: String = Bundle.main.bundleIdentifier ?? ""
  ...
  internal func log(with error: Error, to logTarget: LogCategory, level: OSLogType = .error) {
  ...
  internal func log(with message: String, to logTarget: LogCategory, level: OSLogType) {
    switch logTarget {
      case .network:
        networkLogger.log(level: level, "\(message)")
